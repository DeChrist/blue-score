...
| P2 | React Testing Library workflow coverage | Setup lock, reset, import confirmation, sequential tab behaviour, and legacy recovery warning are not covered by automated UI tests. PR3 score UI predicates and formatting are now partly covered by pure `scoreUiState` tests, including touched-vs-pending state, 12-12 recorded handling, submit gating, pending labels, and next-open rota selection. Remaining unexercised component behavior includes remount-safe undo, keyboard submit, arrow stepping, and focused-only wheel changes. `StandingsTable` now has focused component coverage (`StandingsTable.test.tsx`) for sort selection and `aria-pressed` state, plus duplicate-DOM-safe queries for its responsive list/table surfaces; sort ordering is also covered by pure `standingsSort.test.ts`. `App.tsx` and the remaining React components still read low in the coverage report. | Add focused component/browser tests for critical workflows whose behavior depends on React rendering, DOM events, timers, and responsive layout. | OK |
...
| — | ~~Review bundled font payload~~ | ~~The mobile-first app ships multiple font files and weights.~~ | Resolved: bundled sans-serif (`@fontsource/fira-sans`) removed; UI text now uses the OS system font stack and only `@fontsource/fira-code` (weights 400/500/700) is bundled for numeric/score text. ADR-005 amended to match. | — |
...
| — | ~~Guard CSV export against spreadsheet formula injection~~ | ~~User-entered player names beginning with `= + - @` execute as formulas when the exported CSV is opened in Excel/Sheets.~~ | Resolved: `csvValue` in `exporters.ts` now tab-prefixes such fields before RFC-4180 quoting; negative test added to `exporters.test.ts`. New ADR-012 documents export as a trust boundary. | — |
