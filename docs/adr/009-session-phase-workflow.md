...
- **Setup** — all session configuration is editable (players, court count, points per court, name). Court count is capped by the active Club configuration in app flows. Rotas may be generated or imported. Generating or importing rotas is the explicit session-start action and immediately transitions to `scoring`.
...
`courtCount` is the only field added to `Session` in this decision. Legacy sessions stored without it default to `3`. Club metadata is not persisted in `Session`; the active app configuration supplies the setup court-count cap at runtime. On restore, if a stored session's rotas fail setup validation (e.g. player list changed since the rotas were generated), rotas and results are cleared and the player list is preserved — avoiding data loss on app update.
...
- Future work — player replacement mid-rota and adding rotas after completion — must fit within or extend this phase model. The model is intentionally minimal; do not add persisted phase fields or timestamps without a new ADR.
